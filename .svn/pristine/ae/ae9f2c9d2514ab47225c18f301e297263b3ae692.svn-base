	...
	template: function() {
		...
		for (var index = 0; index < this.pwdArr.length; index++) {
			...
			if (typeof _index == "number") {
				...
				temp2 += '<span class="pwd_btn pwd_line pwd_gray ' + (_index == '删除' ? 'pwd_delete' : '') + '" data-item=' +
					_index + "></span>"
			}
		}
		var html = '<div class="pwd_fixed" id="pwd_fixed"><div class="pwd_box">' +
			'<div class="pwd_title"><span class="pwd_close"></span><span>请输入支付密码</span></div>' +
			'<div class="pwd_input pwd_line">' + temp + "</div>" + '<div class="pwd_num">' + temp2 + "</div>" + "</div></div>";
